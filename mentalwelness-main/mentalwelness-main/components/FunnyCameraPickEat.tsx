...
export default function FunnyCameraPickEat({ onAddXp }: FunnyCameraPickEatProps) {
  ...
  const cardClass =
    "bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden";

  return (
    <div className={`${cardClass} min-h-[480px]`}>
        ...
        <div className="relative bg-zinc-950 min-h-[280px] lg:min-h-[480px] flex items-center justify-center overflow-hidden">
          {phase !== "idle" && cameraOn ? (
            ...
            <div className="text-center p-8 space-y-4">
              ...
              <p className="text-zinc-400 text-sm font-semibold max-w-xs">
          ...
          {phase === "playing" || phase === "eating" ? (
            <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-20">
              ...
              <button
                ...
                className="text-[10px] font-bold bg-amber-500/80 text-white px-2.5 py-1 rounded-full backdrop-blur-sm hover:bg-amber-500 transition"
          ...
          {phase !== "idle" && !cameraOn && !cameraError && (
            <button
              type="button"
              onClick={startCamera}
              className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-white/90 text-zinc-900 text-xs font-bold px-4 py-2 rounded-xl shadow-lg"
            >
        ...
        <div className="p-6 flex flex-col justify-center min-h-[280px]">
          {phase === "idle" && (
            <div className="text-center space-y-5">
              ...
              <button
                type="button"
                onClick={startGame}
                className="bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-600 hover:to-rose-600 text-white font-bold py-3 px-8 rounded-xl text-sm inline-flex items-center gap-2"
              >
          ...
          {phase === "playing" && currentRound && (
            <div className="space-y-5">
              <div>
                ...
                <h4 className="text-base font-extrabold text-zinc-800 dark:text-zinc-100">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {currentRound.options.map((opt) => (
                  <button
                    ...
                    className="p-4 rounded-2xl border-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 hover:border-orange-400 dark:hover:border-orange-500 hover:bg-orange-500/5 transition text-left group"
                  >
                    <span className="text-3xl block mb-1 group-hover:scale-110 transition-transform" aria-hidden="true">
                      {opt.emoji}
                    </span>
                    <span className="text-xs font-bold text-zinc-700 dark:text-zinc-200">{opt.label}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {phase === "eating" && picked && (
            <div className="text-center space-y-4 animate-fadeIn">
              <span className="text-6xl" aria-hidden="true">{picked.emoji}</span>
            ...
            <div className="text-center space-y-4">
              ...
              <div className="flex gap-3 justify-center pt-2">
                <button
                  type="button"
                  onClick={startGame}
                  className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2.5 px-6 rounded-xl text-xs inline-flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 font-bold py-2.5 px-6 rounded-xl text-xs inline-flex items-center gap-1.5"
                >
